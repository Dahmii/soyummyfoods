...
import { generateAdminReceiptPdf, getAdminOrder, getAdminOrderPaymentSummary, getAdminOrderReceiptArtifactStatus, listAdminOrderItems, listAdminOrderStatusHistory, transitionAdminOrderStatus } from '../../repositories/adminOrderRepository';
import type { AdminOrderDetail, AdminOrderItem, AdminOrderPaymentSummary, AdminOrderReceiptArtifactStatus, AdminOrderStatus, AdminOrderStatusHistory, AdminOperationalPaymentStatus } from '../../types/adminOrders';
import { formatMinorCurrency, formatPrice } from '../../utils/currency';
...
function paymentStatusLabel(status: AdminOperationalPaymentStatus, requiresManagerReview: boolean): string {
  if (status === 'succeeded') return 'Paid';
  if (status === 'awaiting_payment_intent') return 'Awaiting payment';
  if (status === 'payment_intent_attached') return 'Pending';
  if (status === 'payment_failed') return 'Failed';
  return requiresManagerReview ? 'Requires review' : 'Review recorded';
}

function providerLabel(provider: AdminOrderPaymentSummary['provider']): string {
  return provider === 'stripe' ? 'Stripe' : provider;
}
...
export function AdminOrderDetailPage() {
  ...
  const [paymentSummary, setPaymentSummary] = useState<AdminOrderPaymentSummary | null>(null);
  ...
  const loadPaymentSummary = useCallback(async () => {
    ...
    try {
      setPaymentSummary(await getAdminOrderPaymentSummary(orderId));
    } catch {
      setPaymentError('Payment information is unavailable.');
    } finally {
  ...
  }, [canAccessReceipts, loadPaymentSummary, orderId]);
  ...
  return <section className="space-y-7">
    ...
    <div className="grid gap-5 lg:grid-cols-2"><section className="rounded-2xl border border-ink/10 bg-white p-5"><h2 className="font-display text-xl font-bold">Totals</h2><dl className="mt-4 space-y-2 text-sm"><div className="flex justify-between"><dt>Subtotal</dt><dd>{formatPrice(Number(order.subtotal))}</dd></div><div className="flex justify-between"><dt>Delivery</dt><dd>{formatPrice(Number(order.delivery_fee))}</dd></div><div className="flex justify-between"><dt>Discount</dt><dd>{formatPrice(Number(order.discount_amount))}</dd></div><div className="flex justify-between"><dt>Tax</dt><dd>{formatPrice(Number(order.tax_amount))}</dd></div><div className="flex justify-between border-t border-ink/10 pt-2 font-bold"><dt>Total</dt><dd>{formatPrice(Number(order.total))} {order.currency_code}</dd></div></dl></section><section className="rounded-2xl border border-ink/10 bg-white p-5"><h2 className="font-display text-xl font-bold">Payment</h2>{paymentLoading ? <p className="mt-4 text-sm text-ink/60">Loading payment information…</p> : paymentError ? <div className="mt-4 text-sm text-amber-800" role="status"><p>{paymentError}</p><Button size="sm" variant="outline" className="mt-3" onClick={() => void loadPaymentSummary()}>Retry</Button></div> : !paymentSummary ? <p className="mt-4 text-sm text-ink/60">No payment attempt recorded.</p> : <><dl className="mt-4 space-y-2 text-sm"><div className="flex justify-between gap-4"><dt className="text-ink/50">Status</dt><dd className={paymentSummary.requires_manager_review ? 'font-semibold text-amber-800' : 'font-medium'}>{paymentStatusLabel(paymentSummary.payment_status, paymentSummary.requires_manager_review)}</dd></div><div className="flex justify-between gap-4"><dt className="text-ink/50">Amount</dt><dd className="font-medium">{formatMinorCurrency(Number(paymentSummary.amount_minor), paymentSummary.currency_code)}</dd></div><div className="flex justify-between gap-4"><dt className="text-ink/50">Provider</dt><dd>{providerLabel(paymentSummary.provider)}</dd></div>{paymentSummary.paid_at ? <div className="flex justify-between gap-4"><dt className="text-ink/50">Paid at</dt><dd className="text-right">{new Date(paymentSummary.paid_at).toLocaleString()}</dd></div> : null}</dl>{paymentSummary.requires_manager_review ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><p className="font-semibold">Payment requires manager review</p><p className="mt-1">This late provider success does not change the order’s operational state.</p>{canCancel ? <Link to="/admin/payments/reconciliation" className="mt-2 inline-block font-semibold underline underline-offset-2">Open reconciliation</Link> : <p className="mt-1">Ask a manager for review.</p>}</div> : null}</>}</section><section className="rounded-2xl border border-ink/10 bg-white p-5"><h2 className="font-display text-xl font-bold">Notes</h2><p className="mt-4 whitespace-pre-wrap text-sm text-ink/70">{order.customer_note ?? 'No customer note.'}</p></section></div>
